...
const transferPath = path.join(dataDir, "musclewiki-plate-transfer.json");
const muscleNames = {
...
for (const [slug, muscleName] of Object.entries(muscleNames)) {
  ...
  const pages = (collection.pages || []).map((page, index) => ({
    number: index + 1,
    url: page.sourcePage || `https://musclewiki.com/exercises/${slug}/plate${index ? `/${index + 1}` : ""}`,
  ...
  const source = `https://musclewiki.com/exercises/${slug}/plate`;
  const baseName = `musclewiki-${slug}-plate`;
