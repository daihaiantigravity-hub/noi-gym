...
const transferPath = path.join(dataDir, "musclewiki-band-transfer.json");
const muscleNames = {
...
for (const [slug, muscleName] of Object.entries(muscleNames)) {
  ...
  const pages = (collection.pages || []).map((page, index) => ({
    number: index + 1,
    url: page.sourcePage || `https://musclewiki.com/exercises/${slug}/band${index ? `/${index + 1}` : ""}`,
  ...
  const source = `https://musclewiki.com/exercises/${slug}/band`;
  const baseName = `musclewiki-${slug}-band`;
