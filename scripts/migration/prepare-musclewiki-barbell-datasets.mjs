...
const transferPath = path.join(dataDir, "musclewiki-barbell-transfer.json");
...
for (const [slug, muscleName] of Object.entries(muscleNames)) {
  ...
  const pages = (collection.pages || []).map((page, index) => ({
    number: index + 1,
    url: page.sourcePage || `https://musclewiki.com/exercises/${slug}/barbell${index ? `/${index + 1}` : ""}`,
  ...
  const source = `https://musclewiki.com/exercises/${slug}/barbell`;
  const baseName = `musclewiki-${slug}-barbell`;
