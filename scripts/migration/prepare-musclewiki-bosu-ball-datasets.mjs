...
const transferPath = path.join(dataDir, "musclewiki-bosu-ball-transfer.json");
const muscleNames = {
...
for (const [slug, muscleName] of Object.entries(muscleNames)) {
  ...
  const pages = (collection.pages || []).map((page, index) => ({
    number: index + 1,
    url: page.sourcePage || `https://musclewiki.com/exercises/${slug}/bosu-ball${index ? `/${index + 1}` : ""}`,
  ...
  const source = `https://musclewiki.com/exercises/${slug}/bosu-ball`;
  const baseName = `musclewiki-${slug}-bosu-ball`;
