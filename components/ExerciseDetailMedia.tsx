...
export default function ExerciseDetailMedia({ imageCount, title }: { imageCount: number; title: string }) {
  const slideCount = Math.max(imageCount, 1);
  ...
  return (
    <div
      aria-label={`${title} demonstration images`}
      className={`exercise-detail-hero-track${slideCount > 1 ? " exercise-detail-hero-track--stacked" : " exercise-detail-hero-track--single"}`}
    ...
    >
      {Array.from({ length: slideCount }, (_, index) => {
        ...
        return (
          <section aria-label={`${title} view ${index + 1}`} className={`exercise-detail-hero ${slide.modifier}`} key={`${title}-${index}`} role="img">
            <span className="exercise-detail-hero__kicker">{slide.kicker}</span>
            <strong className="exercise-detail-hero__title">{slide.title}</strong>
            <span className="exercise-detail-hero__sub">{slide.sub}</span>
            <span aria-hidden="true" className="exercise-detail-hero__person" />
          </section>
        );
      })}
    </div>
